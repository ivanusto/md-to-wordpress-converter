...
- 🛡️ **AI Invisible Watermarks & Fingerprint Auto-Cleaner (AI 隱形浮水印自動清除)**: Automatically strips zero-width spaces (U+200B/C/D), tag characters, bidi overrides, and variation selectors generated by ChatGPT, Claude, and Gemini during `.md` file import or live conversion. Load-bearing invisibles are kept: emoji ZWJ/VS sequences (👨‍👩‍👧), subdivision flags (🏴󠁧󠁢󠁳󠁣󠁴󠁿), ZWNJ/ZWJ inside Arabic, Indic, Khmer or Mongolian words, CJK variation sequences, and RTL directional marks — the same rules as [`text_unicode.py`](https://github.com/guillaumemeyer/watermarks-remover) upstream.
