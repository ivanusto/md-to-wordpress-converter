...
- 🛡️ **AI Invisible Watermarks & Fingerprint Auto-Cleaner (AI 隱形浮水印自動清除)**: A full port of Layer A from [watermarks-remover](https://github.com/guillaumemeyer/watermarks-remover)'s `text_unicode.py`, run during `.md` file import or live conversion. Strips zero-width characters (U+200B/C/D), tag characters, bidi overrides, variation selectors, private-use characters and every other format (`Cf`) character generated by ChatGPT, Claude and Gemini, and normalizes space homoglyphs.
  ...
  - **Optional, off by default**: Latin confusable replacement (Cyrillic/fullwidth → ASCII), NFKC normalization, and two paranoid switches that strip the joiners and RTL marks anyway when you suspect a watermark hides inside them.
