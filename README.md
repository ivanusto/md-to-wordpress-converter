...
- **A truncated ID3v2 tag is reported, and the audio survives it (截斷的 ID3v2 標籤會被回報，音訊留得住)**: mirrors guillaumemeyer/watermarks-remover#201. An MP3 whose tag header declares more tag than the file holds used to parse to nothing and be reported clean. It is now reported as truncated, with whatever markers the surviving bytes carry, and a clean drops the unreadable tag up to the first valid MPEG frame header, checking version, layer, bitrate, sample rate and emphasis rather than trusting a sync word alone. A file with no such header is preserved exactly as it was rather than emptied. Both drivers carry it, and the slice driver finds the frame header without reading the file into memory.
- **`npm run regen:golden` works on Windows**: it imported `tests/samples.ts` by absolute path, and an absolute Windows path starts with a drive letter that `import()` reads as a URL scheme and refuses.
- New anchors: `unmark-web/js/image_meta.js` `b9e4f0f4bc8a`, `unmark-web/js/av_meta.js` `65a3768021b4`, `watermarks-remover/service/scripts/image_meta.py` `bd5d9f19f370`, `watermarks-remover/service/scripts/av_meta.py` `f012ed17276c`.
